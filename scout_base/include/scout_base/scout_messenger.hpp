...
public:
    ScoutMessenger(std::shared_ptr<ScoutType> scout, rclcpp::Node* node) : scout_(scout), node_(node) {}
    ...
    void SetupSubscription()
    {
        ...
        status_pub_ = node_->create_publisher<scout_msgs::msg::ScoutStatus>("/scout_status", 10);

        // cmd subscriber
        motion_cmd_sub_ = node_->create_subscription<geometry_msgs::msg::Twist>("/cmd_vel", 5, std::bind(&ScoutMessenger::TwistCmdCallback, this, std::placeholders::_1));
        light_cmd_sub_ = node_->create_subscription<scout_msgs::msg::ScoutLightCmd>("/light_control", 5, std::bind(&ScoutMessenger::LightCmdCallback, this, std::placeholders::_1));
    ...
    void PublishStateToROS()
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            status_msg.actuator_states[motor_id].rpm = actuator.actuator_hs_state[i].rpm;
            status_msg.actuator_states[motor_id].current = actuator.actuator_hs_state[i].current;
            ...
            status_msg.actuator_states[motor_id].driver_voltage = actuator.actuator_ls_state[i].driver_voltage;
            status_msg.actuator_states[motor_id].driver_temperature = actuator.actuator_ls_state[i].driver_temp;
            status_msg.actuator_states[motor_id].motor_temperature = actuator.actuator_ls_state[i].motor_temp;
        ...
        status_msg.rear_light_state.custom_value = state.light_state.rear_light.custom_value;
...
private:
    ...
    rclcpp::Subscription<scout_msgs::msg::ScoutLightCmd>::SharedPtr light_cmd_sub_;
    ...
    template<typename T, std::enable_if_t<!std::is_base_of<ScoutMiniOmniRobot, T>::value, bool> = true>
    ...
    void PublishOdometryToROS(const MotionStateMessage& msg, double dt)
    {
        ...
        double linear_speed = msg.linear_velocity / 1.2;
        double angular_speed = msg.angular_velocity / 1.2;
        double lateral_speed = 0;

        if (std::is_base_of<ScoutMiniOmniRobot, ScoutType>::value) {
            lateral_speed = msg.lateral_velocity / 1.2;
        }
        else {
            lateral_speed = 0;
        }
