...
{
ScoutBaseRos::ScoutBaseRos(std::string node_name) : rclcpp::Node(node_name), keep_running_(false)
{
...
void ScoutBaseRos::LoadParameters()
{
    ...
    std::cout << "- is scout mini: " << std::boolalpha << is_scout_mini_ << std::endl;
    std::cout << "- is omni wheel: " << std::boolalpha << is_omni_wheel_ << std::endl;
...
bool ScoutBaseRos::Initialize()
{
    ...
    if (detector.Connect(port_name_)) {
        ...
        if (proto == ProtocolVersion::AGX_V1) {
            ...
            if (!is_omni_wheel_) {
                is_omni_ = false;
                robot_ = std::make_shared<ScoutRobot>(ProtocolVersion::AGX_V1, is_scout_mini_);
                if (is_scout_mini_) {
                    std::cout << "Creating interface for Scout Mini with AGX_V1 Protocol" << std::endl;
                }
                else {
                    std::cout << "Creating interface for Scout with AGX_V1 Protocol" << std::endl;
                }
            }
            else {
                is_omni_ = true;
                omni_robot_ = std::unique_ptr<ScoutMiniOmniRobot>(new ScoutMiniOmniRobot(ProtocolVersion::AGX_V1));
                std::cout << "Creating interface for Scout Mini Omni with AGX_V1 Protocol" << std::endl;
            }
        }
            ...
            if (!is_omni_wheel_) {
                is_omni_ = false;
                robot_ = std::make_shared<ScoutRobot>(ProtocolVersion::AGX_V2, is_scout_mini_);
                std::cout << "Creating interface for Scout with AGX_V2 Protocol" << std::endl;
            }
            else {
                is_omni_ = true;
                omni_robot_ = std::unique_ptr<ScoutMiniOmniRobot>(new ScoutMiniOmniRobot(ProtocolVersion::AGX_V2));
                std::cout << "Creating interface for Scout Mini Omni with AGX_V2 Protocol" << std::endl;
            }
        }
        else {
...
void ScoutBaseRos::Run()
{
    ...
    if (!is_omni_) {
        std::unique_ptr<ScoutMessenger<ScoutRobot>> messenger = std::unique_ptr<ScoutMessenger<ScoutRobot>>(new ScoutMessenger<ScoutRobot>(robot_, this));
        ...
        std::unique_ptr<ScoutMessenger<ScoutMiniOmniRobot>> messenger = std::unique_ptr<ScoutMessenger<ScoutMiniOmniRobot>>(new ScoutMessenger<ScoutMiniOmniRobot>(omni_robot_, this));
